...
import {
  getAllArticles,
  getArticleBySlug,
  getArticleClusterForSlug,
  getHeadingsFromContent,
  splitArticleAndComments,
} from "@/lib/articles";
import { formatArticleDate } from "@/lib/articleDate";
import { ArticleBody } from "@/components/ArticleBody";
import { ArticleToc } from "@/components/ArticleToc";
import { ArticleComments } from "@/components/ArticleComments";
import { ArticleDetailRelatedNav } from "@/components/ArticleDetailRelatedNav";
...
export default async function ArticlePage({ params }: PageProps) {
  ...
  const dateFormatted = formatArticleDate(article.meta.date);
  ...
  const cluster = getArticleClusterForSlug(slug);

  const clusterTop =
    cluster != null ? (
      <div className="mb-8 md:mb-10">
        <ArticleDetailRelatedNav hub={cluster.hub} children={cluster.children} currentSlug={slug} />
      </div>
    ) : null;

  const clusterBottom =
    cluster != null ? (
      <div className="mt-12 md:mt-16 mb-0">
        <ArticleDetailRelatedNav hub={cluster.hub} children={cluster.children} currentSlug={slug} />
      </div>
    ) : null;
